...
//import Testing.Cache;
//import Testing.Kernel;
//import Testing.ONE_CLASS_Q;
//import Testing.QMatrix;
//import Testing.SVC_Q;
//import Testing.SVR_Q;
//import Testing.Solver;
//import Testing.Solver_NU;
...
public class SVMCROSS {
   ...
   public static final int LIBSVM_VERSION=300;
                    ...
                    double Cp, double Cn)
    {
        ...
        if (Cp==Cn)
            SVMCROSS.info("nu = "+sum_alpha/(Cp*prob.l)+"\n");
                    ...
                    double[] alpha, Solver.SolutionInfo si)
    {
      ...
      //  SVMCROSS.info("nu = "+sum_alpha/(param.C*l)+"\n");
    }
 ...
 //       SVMCROSS.info("obj = "+si.obj+", rho = "+si.rho+"\n");
      ...
      //  SVMCROSS.info("nSV = "+nSV+", nBSV = "+nBSV+"\n");
                  ...
                  double[] probAB)
    {
        ...
        for (iter=0;iter<max_iter;iter++)
        {
            ...
            {
                SVMCROSS.info("Line search fails in two-class probability estimates\n");
                break;
            }
        }

        if (iter>=max_iter)
            SVMCROSS.info("Reaching maximal iterations in two-class probability estimates\n");
    ...
    private static void multiclass_probability(int k, double[][] r, double[] p)
    {
        ...
        if (iter>=max_iter)
            SVMCROSS.info("Exceeds max_iter in multiclass_prob\n");
    }
    ...
    private static double svm_svr_probability(svm_problem prob, svm_parameter param)
    {
        ...
        //from me
        String y="";
        svm_problem prob1 = null;
        svm_cross_validation(prob, prob1,newparam,nr_fold,ymv, y,10);
        for(i=0;i<prob.l;i++)
        {
        ...
        for(i=0;i<prob.l;i++)
            if (Math.abs(ymv[i]) > 5*std)
                ...
                mae+=Math.abs(ymv[i]);
        mae /= (prob.l-count);
        SVMCROSS.info("Prob. model for test data: target value = predicted value + z,\nz: Laplace distribution e^(-|z|/sigma)/(2sigma),sigma="+mae+"\n");
        return mae;
    }
    ...
    public static void svm_cross_validation(svm_problem prob,svm_problem SmallData, svm_parameter param, int nr_fold, double[] target,String Fselected,int AllGenes )
    {
    	//param.gamma =0;
    //	param.C=0;
    ...
    //	System.out.println(param.C+ "          "+param.gamma  + " svm type    "+ param.svm_type+ "  k "+param.kernel_type +"  No.of Flods ::"+nr_fold);
        int i;
        ...
        Vector<svm_node[]> vx = new Vector<svm_node[]>();  // store values and indexes
        Vector<Double> vy = new Vector<Double>();         // Store class label values
                
        
        ArrayList<svmsize>  SetInstances=new ArrayList<svmsize>(); 
        int countinstance=0;
        ArrayList Felement =new ArrayList() ;
        

        int counts=0,variableValue,Findex;
        String next="", lastfeature="";
        ...
        StringTokenizer str = new StringTokenizer(Fselected,",");
           ...
           while (str.hasMoreTokens())
              ...
              //add double to array
                Felement.add(counts,variableValue);
                counts++;
               }
        ...
        else
        {
            ...
            for(i=0;i<l;i++)
            {
                ...
                do {int _=perm[i]; perm[i]=perm[j]; perm[j]=_;} while(false);
                // Rearrange perm with different index
            }
            for(i=0;i<=nr_fold;i++)
     ...
     //   System.out.println(" Randomly ");
       /* for(i=0;i<nr_fold;i++) 
        	  ...
        	  System.out.println(fold_start[i]+ " End  "+fold_start[i+1] +"   "+ (fold_start[i+1]-fold_start[i]));
        
        */
        
        
        int j,k,begin=0,end=0;
        svm_problem subprob;
        svm_problem Testprob;
        int max_index=0;
        Double  yclass;
        svm_model submodel;
        svm_problem  probcross;
        ...
        for(int Fo=0;Fo<nr_fold;Fo++)
        {
            ...
            Testprob.x = new svm_node[Testprob.l][];
            Testprob.y = new double[Testprob.l];
            
            
            k=0;
            
          //  System.out.println("Hellooooo == (end-begin)="+(end-begin));
         //   System.out.println("      fold    "+(i+1));
    
            for(j=begin;j<end;j++)
            {
            	Testprob.x[k] = SmallData.x[perm[j]];
            	Testprob.y[k] = SmallData.y[perm[j]];
                
                // give the subprobx[k] the whole line in different distribution generate from permit
                // then pass it, here Geneselection in the next step
                
                // perm for randomly instances prob.x[] to get all the record
          ...
          //  System.out.println("      Training      ");
            for(j=0;j<begin;j++)
            {
                subprob.x[k] = SmallData.x[perm[j]];
                subprob.y[k] = SmallData.y[perm[j]];
              //  System.out.println(perm[j]);
   ...
   //         System.out.println("  End    Training     ");
            //  my updates for cross validation depend on feature selection
            
        //    System.out.println( " Round    "+ Fo); ;    
        ...
        /* begin=0;
         ...
         Testprob.y = new double[Testprob.l];*/
            ...
            for(j=0 ;j<k;j++)                 //  K contains the training instances and doesn't contain values in testing (Begin-End)
    	
    	     {
            
            			svm_node[] x1 ;//= new svm_node[Orgnodesize];
            			...
            			yclass=(Double)subprob.y[j];
            			vy.addElement(yclass);
            			...
            			svm_node[] x = new svm_node[Felement.size()];
		            for(int s=0;s<x1.length;s++)  {
	            	
		            	
		            	Findex=Integer.valueOf(x1[s].index);
		             
		            	if(Felement.contains(Findex)){
	            		
	            		    x[countinstance] = new svm_node();
			                x[countinstance].index = Findex;
			                instvalue= Double.valueOf(x1[s].value);
			                x[countinstance].value =instvalue;
	            	   	//	System.out.println("s==  "+s + "  countinstance"+countinstance);
	            	   		countinstance++;
	                	}
		            ...
		            int Orgnodesize=Felement.size();
			  
		/*
				         if(Orgnodesize>0) 
				        	  	max_index = Math.max(max_index, x[Orgnodesize-1].index);
				           */ 	//max_index = Math.max(max_index, x[Snodesize-1].index);
	            
	            	            
	            vx.addElement(x);
	           	            
            }//end K  for fold finished
       ...
       /* subprob = new svm_problem();
        ...
        Testprob.y = new double[Testprob.l];
            */
        	...
        	probcross = new svm_problem();
        	probcross.l = vy.size();
        	probcross.x = new svm_node[probcross.l][];
        
        for(int a=0;a<probcross.l;a++)
        	probcross.x[a] = vx.elementAt(a);
       
        	probcross.y = new double[probcross.l];
        for(int b=0;b<probcross.l;b++)
        	probcross.y[b] = vy.elementAt(b);
        // from me must delete
        
        	
        submodel = svm_train(probcross,param);
	        ...
	        if(param.kernel_type == svm_parameter.PRECOMPUTED)
	            for(int w=0;w<prob.l;w++)
	            {
	                if (prob.x[w][0].index != 0)
	                {
	                    System.err.print("Wrong kernel matrix: first column must be 0:sample_serial_number\n");
	                    System.exit(1);
	                }
	                if ((int)prob.x[w][0].value <= 0 || (int)prob.x[w][0].value > max_index)
	                {
           ...
           // svm_model submodel = svm_train(subprob,param);
        	...
        	// Reduce Testing data to be mapped with only selected feature      
  yclass=0.0;
   ...
   /*     String contact="";
        		 System.out.println(" Testing data All    "+ Testprob.l);
        		 
        		 System.out.println(" Begin "+begin+"  "+end);
        		 
        		 int count=0;
           for(j=begin ;j<end;j++)                 //  K contains the training instances and doesn't contain values in testing (Begin-End)
           	
  	     {
        
        	   svm_node[] x2 ;//= new svm_node[Orgnodesize];

           	
           	x2=Testprob.x[count];
           	contact+="\n";
            for(int s=0;s<x2.length;s++)  {
            	
  	            		            	
  		            Findex=Integer.valueOf(x2[s].index);
  		          instvalue= Double.valueOf(x2[s].value);
  		    //    contact+= Findex +" :: "+ instvalue ;
  		        contact+= Findex +" :: " ;
  		      contact+= instvalue +" :: " ;
  	     }
       ...
       //  System.out.println(" end-- begin = "+(end-begin));
         int count=0;
            for(j=begin ;j<end;j++)                 //  K contains the training instances and doesn't contain values in testing (Begin-End)
    	
    	     {
            	...
            	yclass=(Double)Testprob.y[count];
                vy.addElement(yclass);
		        ...
		        countinstance=0;
		           ...
		           svm_node[] Tx = new svm_node[Felement.size()];
		          // System.out.println("Felemet length==  "+Felement.size());
		            for(int s=0;s<x2.length;s++)  {
	            	
	          // Assign only values related to the Gene_subset.
		            	
		            Findex=Integer.valueOf(x2[s].index);
	            	if(Felement.contains(Findex)){
	            		
	            		Tx[countinstance] = new svm_node();
	            		Tx[countinstance].index = Findex;
			                instvalue= Double.valueOf(x2[s].value);
			                Tx[countinstance].value =instvalue;
	            	   	//	System.out.println("s==  "+s + "  countinstance"+countinstance);
	            	   		countinstance++;
	             	}
		...
		/*
				         if(Orgnodesize>0) 
				        	  	max_index = Math.max(max_index, Tx[Orgnodesize-1].index);
				            */	//max_index = Math.max(max_index, x[Snodesize-1].index);
            ...
            svm_problem  Tcross = new svm_problem();
            Tcross.l = vy.size();
            Tcross.x = new svm_node[Tcross.l][];
            
	        for(int a=0;a<Tcross.l;a++)
	        	Tcross.x[a] = vx.elementAt(a);
	       
	        Tcross.y = new double[Tcross.l];
	        for(int b=0;b<Tcross.l;b++)
	        	Tcross.y[b] = vy.elementAt(b);
	        // from me must delete
          ...
          // ********************   Very important statement  ******************
           ...
           target[perm[j]] = svm_predict(submodel,Tcross.x[count]); ///  
           		
  //    target[perm[j]] = svm_predict(submodel,prob.x[perm[j]]); 
           		
           	//	very important
            		//target[perm[j]] =3.0;
         // System.out.println(svm_predict(submodel,Tcross.x[count])  );
           		
           		
  // target[perm[j]] = svm_predict(submodel,SmallData.x[perm[j]]); ///     very important
            	count++;
...
System.out.println("degree "+param.degree);
    ...
    public static String svm_check_parameter(svm_problem prob, svm_parameter param)
    {
    	...
    	System.out.println("   SvM  type    "+param.svm_type+ "   Svm kernal "+param.kernel_type);
...
// hidden function of print in this code
   ...
   * 
    ...
    I  Override this function due to the the invoking occures inside svm_svr_probability Block
     private static double svm_svr_probability(svm_problem prob, svm_parameter param)
   ...
   */
   ...
   //**************************************** Be carefull this for probability ***************************************************
   /*
   public static void svm_cross_validation(svm_problem prob, svm_parameter param, int nr_fold, double[] target,String Fselected )
   ...
   // *****************************************************************************************************************************
   {
     ...
     //  int size=41;  // IDS 41 feature without class label
    //   char ChFeature []=new char[size];
   //    ChFeature=Fselected.toCharArray();
       ...
       // get the position index of features
    ...
    // Insert poistions into Arraylist
       ...
       for(i=0;i<nr_fold;i++)
       {
   	...
   	
   	     {
	          ...
	          // Assign only values related to the Gene_subset.
	            	...
	            	if(Felement.contains(Findex)){
	            		...
	            		instvalue= Double.valueOf(x1[s].value);
	            		SetInstances.add(countinstance,new svmsize(Findex,instvalue));
	            		countinstance++;
	                	}
		            ...
		            int Orgnodesize=SetInstances.size();
			  
			            
			           svm_node[] x = new svm_node[Orgnodesize];
			          
			           int iter=0;
			           
			            for(int c=0;c<Orgnodesize;c++)  {
			            	
			                x[c] = new svm_node();
			                x[c].index = SetInstances.get(c).getSindex();

			                x[c].value =SetInstances.get(c).getSValue();
           ...
           svm_problem  probcross = new svm_problem();
           probcross.l = vy.size();
           probcross.x = new svm_node[probcross.l][];
          ...
          // svm_model submodel = svm_train(subprob,param);
           ...
           svm_model submodel = svm_train(probcross,param);
         ...
         // ********************   Very important statement  ******************
           	...
           	for(j=begin;j<end;j++)
                   ...
                   target[perm[j]] = svm_predict(submodel,prob.x[perm[j]]); ///     very important
